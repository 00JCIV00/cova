...
pub fn Typed(comptime SetT: type, comptime config: Config) type {
    return struct {
        ...
        pub fn get(self: *const @This()) !ChildT {
            return if (self.is_set and !self.is_empty)
                self._set_args[0].?
            else if (self.default_val) |def_val|
                def_val
            else if (ChildT == bool) false else error.ValueNotSet;
        }
    ...
    // Custom Implementation
        ...
        if (config.use_custom_bit_width_range) {
            ...
            inline for (config.min_int_bit_width..config.max_int_bit_width) |bit_width| {
                ...
                tag_info.fields = tag_info.fields ++ [_]builtin.Type.UnionField{.{ .name = uint_name, .value = tag_info.fields.len }};
                ...
                tag_info.fields = tag_info.fields ++ [_]builtin.Type.UnionField{.{ .name = int_name, .value = tag_info.fields.len }};
            }
        } else {
