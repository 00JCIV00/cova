...
pub fn Typed(comptime SetT: type, comptime config: Config) type {
    return struct {
        ...
        /// A Validation Function that additionally receives `std.Io`.
        ///
        /// This is intended for validation that needs I/O, such as checking file paths.
        valid_fn_io: ?*const fn (ChildT, mem.Allocator, Io) bool = null,
        ...
        pub const child_type_parse_fn: ?*const fn ([]const u8, mem.Allocator) anyerror!ChildT = typeParseFn: {
            for (config.child_type_parse_fns orelse break :typeParseFn null) |elm| {
                if (elm.ChildT == SetT) break :typeParseFn @as(*const fn ([]const u8, mem.Allocator) anyerror!ChildT, @ptrCast(@alignCast(elm.parse_fn)));
        ...
        pub const child_type_help_fn: ?*const fn ([]const u8, ?mem.Allocator) anyerror!ChildT = typeHelpFn: {
            for (config.child_type_help_fns orelse break :typeHelpFn null) |elm| {
                if (elm.ChildT == SetT) break :typeHelpFn @as(*const fn (anytype, anytype, ?mem.Allocator) anyerror!ChildT, @ptrCast(@alignCast(elm.help_fn)));
        ...
        pub const child_type_usage_fn: ?*const fn ([]const u8, ?mem.Allocator) anyerror!ChildT = typeUsageFn: {
            for (config.child_type_usage_fns orelse break :typeUsageFn null) |elm| {
                if (elm.ChildT == SetT) break :typeUsageFn @as(*const fn (anytype, anytype, ?mem.Allocator) anyerror!ChildT, @ptrCast(@alignCast(elm.usage_fn)));
        ...
        pub fn parse(self: *const @This(), arg: []const u8) !ChildT {
            ...
            return switch (@typeInfo(ChildT)) {
                .bool => isTrue: {
                    ...
                    for (true_words[0..]) |word| {
                        if (mem.eql(u8, word, san_arg)) break :isTrue true;
                    } else break :isTrue false;
                },
                .pointer => arg,
        ...
        pub fn set(self: *const @This(), set_arg: []const u8) !void {
            try self.setContext(set_arg, null);
        }

        /// Set this Value with an `std.Io` instance available to validation functions.
        pub fn setWithIo(self: *const @This(), set_arg: []const u8, io: Io) !void {
            try self.setContext(set_arg, io);
        }

        fn setContext(self: *const @This(), set_arg: []const u8, io: ?Io) !void {
            // Delimited Args
            var arg_delim: u8 = ' ';
            ...
            if (self.set_behavior == .Multi and meta.activeTag(@typeInfo(ChildT)) != .pointer and check_delim) {
                ...
                while (split_args.next()) |arg| try self.setContext(arg, io);
                return;
            }

            // Single Arg
            const parsed_arg = try self.parse(set_arg);
            @constCast(self).is_set = try self.validate(parsed_arg, io);
            if (self.is_set) {
        ...
        fn validate(self: *const @This(), parsed_arg: ChildT, io: ?Io) !bool {
            if (self.valid_fn == null and self.valid_fn_io == null) return true;
            const alloc = self._alloc orelse {
                log.err("The Value '{s}' does not have an Allocator!", .{self.name});
                return error.ValueNotInitialized;
            };
            if (self.valid_fn) |validFn| {
                if (!validFn(parsed_arg, alloc)) return false;
            }
            if (self.valid_fn_io) |validFnIo| {
                const io_ctx = io orelse {
                    log.err("The Value '{s}' requires an Io instance for validation.", .{self.name});
                    return error.IoRequiredForValidation;
                };
                if (!validFnIo(parsed_arg, alloc, io_ctx)) return false;
            }
            return true;
        }
        ...
        pub fn get(self: *const @This()) !ChildT {
            return if (self.is_set and !self.is_empty) self._set_args[0].? else if (self.default_val) |def_val| def_val else if (ChildT == bool) false else error.ValueNotSet;
        }
...
pub fn Generic(comptime config: Config) type {
    ...
    // Custom Implementation
        ...
        const base_union_info = @typeInfo(base_union).@"union";
        const base_tag_info = @typeInfo(base_union_info.tag_type.?).@"enum";

        var field_names: []const []const u8 = &.{};
        var field_types: []const type = &.{};
        var field_attrs: []const builtin.Type.UnionField.Attributes = &.{};
        var tag_values: []const usize = &.{};

        for (base_union_info.fields, base_tag_info.fields) |u_fld, e_fld| {
            field_names = field_names ++ [_][]const u8{u_fld.name};
            field_types = field_types ++ [_]type{u_fld.type};
            field_attrs = field_attrs ++ [_]builtin.Type.UnionField.Attributes{.{}};
            tag_values = tag_values ++ [_]usize{e_fld.value};
        }
        ...
        if (config.use_custom_bit_width_range) {
            ...
            inline for (config.min_int_bit_width..config.max_int_bit_width) |bit_width| {
                ...
                field_names = field_names ++ [_][]const u8{uint_name};
                field_types = field_types ++ [_]type{uint_type};
                field_attrs = field_attrs ++ [_]builtin.Type.UnionField.Attributes{.{}};
                tag_values = tag_values ++ [_]usize{tag_values.len};

                const int_name = @typeName(meta.int(.signed, bit_width));
                const int_type = Typed(meta.int(.signed, bit_width), config);
                field_names = field_names ++ [_][]const u8{int_name};
                field_types = field_types ++ [_]type{int_type};
                field_attrs = field_attrs ++ [_]builtin.Type.UnionField.Attributes{.{}};
                tag_values = tag_values ++ [_]usize{tag_values.len};
            }
        } //
            ...
            if (config.add_base_ints) {
                ...
                const int_fields_start = field_names.len;
                for (int_info.fields, 0..) |u_fld, i| {
                    field_names = field_names ++ [_][]const u8{u_fld.name};
                    field_types = field_types ++ [_]type{u_fld.type};
                    field_attrs = field_attrs ++ [_]builtin.Type.UnionField.Attributes{.{}};
                    tag_values = tag_values ++ [_]usize{int_fields_start + i};
                }
            }
            if (config.add_base_floats) {
                ...
                const float_fields_start = field_names.len;
                for (float_info.fields, 0..) |u_fld, i| {
                    field_names = field_names ++ [_][]const u8{u_fld.name};
                    field_types = field_types ++ [_]type{u_fld.type};
                    field_attrs = field_attrs ++ [_]builtin.Type.UnionField.Attributes{.{}};
                    tag_values = tag_values ++ [_]usize{float_fields_start + i};
                }
            }
        }
        ...
        for (config.custom_types) |T| {
            ...
            const add_name: []const u8 = @typeName(AddT.ChildT);
            const add_type: type = AddT;
            const add_tag: usize = field_names.len + adds;
            for (field_names, 0..) |name, idx| {
                if (!mem.eql(u8, name, add_name)) continue;
                adds += 1;
                field_types = rebuildTypes: {
                    var rebuild: [field_types.len]type = undefined;
                    for (rebuild[0..], field_types, 0..) |*r_fld, o_fld, r_idx|
                        r_fld.* = if (r_idx == idx) add_type else o_fld;
                    const rebuild_out = rebuild;
                    break :rebuildTypes rebuild_out[0..];
                };
                tag_values = rebuildTags: {
                    var rebuild: [tag_values.len]usize = undefined;
                    for (rebuild[0..], tag_values, 0..) |*r_fld, o_fld, r_idx|
                        r_fld.* = if (r_idx == idx) add_tag else o_fld;
                    const rebuild_out = rebuild;
                    break :rebuildTags rebuild_out[0..];
                };
                break;
            } //
            else {
                field_names = field_names ++ [_][]const u8{add_name};
                field_types = field_types ++ [_]type{add_type};
                field_attrs = field_attrs ++ [_]builtin.Type.UnionField.Attributes{.{}};
                tag_values = tag_values ++ [_]usize{add_tag};
            }
        }

        var types_arr: [field_types.len]type = undefined;
        for (&types_arr, field_types) |*dst, src| dst.* = src;
        var attrs_arr: [field_attrs.len]builtin.Type.UnionField.Attributes = undefined;
        for (&attrs_arr, field_attrs) |*dst, src| dst.* = src;
        var tag_arr: [tag_values.len]usize = undefined;
        for (&tag_arr, tag_values) |*dst, src| dst.* = src;

        const TagEnum = @Enum(usize, .exhaustive, field_names, &tag_arr);
        break :customUnion @Union(.auto, TagEnum, field_names, &types_arr, &attrs_arr);
    };
}
    ...
    return struct {
        ...
        pub fn getAs(self: *const @This(), comptime T: type) !T {
            return switch (meta.activeTag(self.*.generic)) {
                inline else => |tag| {
                    ...
                    return if (@TypeOf(typed_val).ChildT == T) try typed_val.get() else if (@typeInfo(T) == .@"enum" or ( //
        ...
        pub fn getAllAs(self: *const @This(), comptime T: type) ![]T {
            return switch (meta.activeTag(self.*.generic)) {
                inline else => |tag| {
                    const typed_val = @field(self.*.generic, @tagName(tag));
                    return if (@TypeOf(typed_val).ChildT == T) try typed_val.getAll() else if (@typeInfo(T) == .@"enum" or ( //
                        @typeInfo(T) == .optional and //
                            @typeInfo(@typeInfo(T).optional.child) == .@"enum" //
                    )) {
        ...
        /// Set the inner Typed Value with an `std.Io` instance available to validation functions.
        pub fn setWithIo(self: *const @This(), arg: []const u8, io: Io) !void {
            switch (meta.activeTag(self.*.generic)) {
                inline else => |tag| try @field(self.*.generic, @tagName(tag)).setWithIo(arg, io),
            }
        }
        ...
        pub fn childTypeName(self: *const @This()) []const u8 {
            ...
            return switch (meta.activeTag(self.*.generic)) {
                inline else => |tag| typeName: {
                    const val = @field(self.*.generic, @tagName(tag));
                    break :typeName if (val.alias_child_type) |alias| alias else if (config.child_type_aliases) |aliases| confAlias: {
        ...
        /// Check if the inner Typed Value has a custom `valid_fn` or `valid_fn_io`.
        pub fn hasCustomValidFn(self: *const @This()) bool {
            return switch (meta.activeTag(self.*.generic)) {
                inline else => |tag| hasFn: {
                    const val = @field(self.*.generic, @tagName(tag));
                    break :hasFn val.valid_fn != null or val.valid_fn_io != null;
                },
            };
        }
        /// Check if the inner Typed Value has a custom `parse_fn`, `valid_fn`, or `valid_fn_io`.
        ...
        pub fn from(comptime from_comp: anytype, from_config: FromConfig) ?@This() {
            const comp_name: []const u8 =
                if (from_config.val_name) |val_name| val_name else switch (@TypeOf(from_comp)) {
            ...
            if (comp_info == .pointer and comp_info.pointer.child != u8) {
                if (!from_config.ignore_incompatible) @compileError("The component '" ++
                    if (comp_name.len > 0) comp_name else "' function parameter of type '" ++
                        @typeName(FromT) ++ "' is incompatible. Pointers must be of type '[]const u8'.") else return null;
            }
            ...
            const CompT = switch (comp_info) {
                ...
                .array => aryType: {
                    ...
                    if (ary_info == .optional) break :aryType ary_info.optional.child else break :aryType comp_info.array.child;
                },
                .@"enum" => |enum_info| EnumT: {
                ...
                .bool, .int, .float, .pointer => FromT,
                    ...
                    if (!from_config.ignore_incompatible) @compileError("The comp '" ++ comp_name ++ "' of type '" ++ @typeName(FromT) ++ "' is incompatible.") else return null;
                },
            };
            ...
            return ofType(CompT, .{
                ...
                .max_entries = if (comp_info == .array) comp_info.array.len else 1,
                .set_behavior = if (comp_info == .array) .Multi else .Last,
                // TODO: Handle default Array Elements.
                .default_val = defVal: {
                    if (utils.indexOfEql([]const u8, meta.fieldNames(@TypeOf(from_comp))[0..], "default_value") != null and
...
pub const ParsingFns = struct {
    ...
    pub const Builder = struct {
        ...
        pub fn altBool(comptime true_words: []const []const u8, comptime false_words: []const []const u8, comptime no_match: BoolNoMatch) fn ([]const u8, mem.Allocator) anyerror!bool {
            return struct {
                fn boolCheck(arg: []const u8, alloc: mem.Allocator) !bool {
                    _ = alloc;
                    for (true_words[0..]) |word| {
                        if (mem.eql(u8, word, arg)) return true;
                    } else for (false_words[0..]) |word| {
                        if (mem.eql(u8, word, arg)) return false;
...
/// Validation Functions for various common requirements to be used with `valid_fn` or `valid_fn_io`.
/// Note, validation functions are in no way limited to these functions.
pub const ValidationFns = struct {
    ...
    pub const Builder = struct {
        ...
        pub fn inRange(comptime NumT: type, comptime start: NumT, comptime end: NumT, comptime inclusive: bool) fn (NumT, mem.Allocator) bool {
            ...
            return if (inclusive) struct {
                fn inRng(num: NumT, alloc: mem.Allocator) bool {
                    _ = alloc;
    ...
    pub fn validFilepath(filepath: []const u8, alloc: mem.Allocator, io: Io) bool {
        _ = alloc;
        Io.Dir.cwd().access(io, filepath, .{}) catch {
            log.warn("The file '{s}' could not be found.", .{filepath});
            return false;
        };
        return true;
    }
...
test "validFilepath checks filesystem access with Io" {
    const testing = std.testing;
    const io = testing.io;
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();

    var file = try tmp.dir.createFile(io, "exists.txt", .{});
    file.close(io);

    var existing_path_buf: [128]u8 = undefined;
    const existing_path = try fmt.bufPrint(
        &existing_path_buf,
        ".zig-cache/tmp/{s}/exists.txt",
        .{tmp.sub_path},
    );
    try testing.expect(ValidationFns.validFilepath(existing_path, testing.allocator, io));

    const old_log_level = testing.log_level;
    testing.log_level = .err;
    defer testing.log_level = old_log_level;

    var missing_path_buf: [128]u8 = undefined;
    const missing_path = try fmt.bufPrint(
        &missing_path_buf,
        ".zig-cache/tmp/{s}/missing.txt",
        .{tmp.sub_path},
    );
    try testing.expect(!ValidationFns.validFilepath(missing_path, testing.allocator, io));
}
