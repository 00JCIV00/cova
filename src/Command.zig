        ...
        fn optShortName(
            ...
            return shortName: {
                for (arg_name) |char| {
                    ...
                    for (ul_chars) |ul| {
                        if (short_idx.* > 0 and (utils.indexOfEql(u8, short_names[0..short_idx.*], ul) != null or
                            utils.indexOfEql(u8, excluded_short_opts[0..], ul) != null)) continue;
                ...
                switch (@typeInfo(param.type.?)) {
                    ...
                    .Array => |ary| {
                        ...
                        switch (ary_info) {
                            // Values
                            .Bool, .Int, .Float, .Optional, .Pointer, .Enum => {
                                from_vals[vals_idx] = ValueT.from(param, .{ .ignore_incompatible = from_config.ignore_incompatible, .val_name = arg_name orelse "val-" ++ .{ '0', (vals_idx + 48) }, .val_description = arg_description }) orelse continue;
                                vals_idx += 1;
                            },
        ...
        fn initCtx(comptime self: *const @This(), comptime init_config: InitConfig, comptime is_root_cmd: bool, parent_cmd: ?*const @This(), init_alloc: mem.Allocator) !if (is_root_cmd) *@This() else @This() {
            ...
            if (help_config.add_help_cmds and (utils.indexOfEql([]const u8, &.{ "help", "usage" }, self.name) == null)) {
                const add_cmd_help_group = switch (help_config.add_cmd_help_group) {
                    ...
                    .Add => add: {
                        init_cmd.cmd_groups =
                            if (init_cmd.cmd_groups) |cmd_groups| try mem.concat(alloc, []const u8, &.{ cmd_groups, &.{help_config.help_group_name} }) else try alloc.dupe([]const u8, &.{help_config.help_group_name});
                        break :add true;
                    },
                    .DoNotAdd => false,
                };
            ...
            if (help_config.add_help_opts) {
                const add_opt_help_group = switch (help_config.add_opt_help_group) {
                    ...
                    .Add => add: {
                        init_cmd.opt_groups =
                            if (init_cmd.opt_groups) |opt_groups| try mem.concat(alloc, []const u8, &.{ opt_groups, &.{help_config.help_group_name} }) else try alloc.dupe([]const u8, &.{help_config.help_group_name});
                        break :add true;
                    },
                    .DoNotAdd => false,
                };
