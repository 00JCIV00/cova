    ...
    return struct {
                        ...
                        for (opts) |opt| {
                            if (mem.eql(u8, opt.name, arg_name)) {
                                if (!opt.val.isSet() and type_info == .@"struct") {
                                    ...
                                    @field(out, field.name) =
                                        if (field.default_value_ptr) |def_val| @as(*field.type, @ptrCast(@alignCast(@constCast(def_val)))).* else null;
                                    break;
                                }
        ...
        fn initCtx(
            ...
            if (help_config.add_help_cmds and (utils.indexOfEql([]const u8, &.{ "help", "usage" }, self.name) == null)) {
                const add_cmd_help_group = switch (help_config.add_cmd_help_group) {
                    ...
                    .Add => add: {
                        init_cmd.cmd_groups =
                            if (init_cmd.cmd_groups) |cmd_groups|
                                try mem.concat(alloc, []const u8, &.{ cmd_groups, &.{help_config.help_group_name} })
                            else
                ...
                init_cmd.sub_cmds = if (init_cmd.sub_cmds != null)
                    try mem.concat(alloc, @This(), &.{ init_cmd.sub_cmds.?, help_sub_cmds[0..] })
                else
                    try alloc.dupe(@This(), help_sub_cmds[0..]);
            }
            ...
            if (help_config.add_help_opts) {
                const add_opt_help_group = switch (help_config.add_opt_help_group) {
                    .AddIfOthers => ifOthers: {
                        if (init_cmd.opt_groups) |opt_groups| {
                            init_cmd.opt_groups = try mem.concat(
                                alloc,
                                []const u8,
                                &.{ opt_groups, &.{help_config.help_group_name} },
                            );
                            break :ifOthers true;
                        }
                        break :ifOthers false;
                ...
                init_cmd.opts =
                    if (init_cmd.opts) |init_opts|
                        try mem.concat(alloc, @This().OptionT, &.{ init_opts, help_opts[0..] })
                    else
                        try alloc.dupe(OptionT, help_opts[0..]);
            }
